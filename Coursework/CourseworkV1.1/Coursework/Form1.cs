...
namespace Coursework
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void btn_sellTile_Click(object sender, EventArgs e)
        {
            ...
            int d = 100;
            Rectangle rectangle1 = new Rectangle(a, b, c, d);
            Point userChoice = new Point(0, 0);
            Point userPos = new Point(MousePosition.X, MousePosition.Y);
            lbl_sell.Visible = true;
            bool tileSelected = false;

            while (tileSelected == false)
            {
                if (userChoice != new Point(0, 0))
                {

                }
                if (new Point(MousePosition.X, MousePosition.Y) != userPos)
                {
                    userChoice = new Point(MousePosition.X, MousePosition.Y);
                    for (int j = 0; j < 9; j++)
                    {

                        for (int i = 0; i < 16; i++)
                        {
                            if (rectangle1.Contains(userPos))
                            {
                                tileSelected = true;
                            }
                            else
